...
- Preview URLs are viewed in browsers and on mobile — use relative units (`%`, `vw`, `vh`, `em`) and `max-width` with `margin: 0 auto` so layouts scale gracefully on any screen
