...
const GraphVisualization = ({ data, onNodeClick, onNodeRightClick }) => {
  ...
  useEffect(() => {
    ...
    const newSimulation = d3.forceSimulation(data.nodes)
      ...
      .strength(d => d.type === 'word' ? -350 * sizeScale(d.dataSize) : -20)
    )
      ...
      .force('collide', d3.forceCollide(d => d.type === 'word' ? sizeScale(d.dataSize) + 15 : 5)) // Sets collision radius: 'word' nodes vary based on size; others have fixed radius (10).
