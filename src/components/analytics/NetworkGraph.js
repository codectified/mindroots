...
export default function NetworkGraph({ data }) {
  ...
  useEffect(() => {
    ...
    // ── draw function — called initially and on resize ───────────────
    const draw = () => {
      const rect = container.getBoundingClientRect();
      const w = Math.round(rect.width)  || container.offsetWidth  || 800;
      const h = Math.round(rect.height) || container.offsetHeight || 600;
      ...
      // reset node positions on resize so they re-center
      nodes.forEach(n => { n.x = undefined; n.y = undefined; n.fx = null; n.fy = null; });
