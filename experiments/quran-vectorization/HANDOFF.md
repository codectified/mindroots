...
- Five experiments done: clustering + k-selection, surah-level geometry,
  whitening, and root-distribution masks. Figures/reports in `out/`.
...
- Committed: `29cef30` (first batch — embed/analyze/kselect/geometry/k2/arrahman
  + early `out/`).
- **NOT yet committed** (working tree): `whiten.py`, `root_export.js`,
  `root_dist.py`, `db_query.js`, `roots.jsonl`, `clusters_whitened.csv`,
  updated `README.md`, this `HANDOFF.md`, and `out/whiten_*` + `out/root_*`.
...
- **Immediate proposed next step (cheap, no new embeddings):** re-run the Exp 5
  root masks over the **whitened** UMAP (`clusters_whitened.csv`) and diff
  localization vs raw — roots that *tighten* under whitening have genuine
  semantic concentration (vs register artifacts).
